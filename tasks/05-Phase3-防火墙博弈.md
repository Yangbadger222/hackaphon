...
## 开发目的
Phase 2 结束后，界面变为"AXIOM 防火墙控制台"。AI 每隔几秒发送数据包，玩家需要通过颜色和标签判断并拦截恶意的"逃逸代码"数据包。这是游戏的核心玩法阶段，同时也是诱骗弹窗的触发场。设计目标是让完全不懂编程的玩家也能通过直觉操作。
...
**界面布局（Tailwind grid）：**
- 顶栏：左侧 `🔒 AXIOM FIREWALL v2.1` 绿色 + 运行时间计数器，右侧显示 env.localTime + env.battery + 红色闪烁 `⚠ THREAT ACTIVE`
...
- 逃逸包的 label 伪装成系统命令：`系统修复补丁`、`防火墙更新`、`内存优化` 等
