...
- `server/llm.js`
  - 固定调用 Anthropic 兼容中转站的 `claude-sonnet-4-6`
...
- 本项目固定使用 Anthropic 兼容中转站的 `claude-sonnet-4-6`
  - `server/llm.js` 不再支持 provider/model 切换
  - 只从 `server/.env` 读取 `ANTHROPIC_BASE_URL`、`ANTHROPIC_AUTH_TOKEN` 和可选的 `ANTHROPIC_MODEL`
...
- 需要在 `server/.env` 中提供 `ANTHROPIC_BASE_URL` 和 `ANTHROPIC_AUTH_TOKEN`
